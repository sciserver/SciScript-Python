...
class FieldReader:
  ...
  def __init__(self, dataset, field, t, twidth, x, y, z, xwidth, ywidth, zwidth, strideTime=1, strideSpace=1,filter=None, token=""):
  ...
  def read(self):
    ...
    if(b_x < 0 or b_y < 0 or b_z < 0 or (b_xwidth > self.dataset.xsize - b_x) or (b_ywidth > self.dataset.ysize - b_y) or (b_zwidth > self.dataset.zsize - b_z)):
      border = 0
      b_x = self.x
    ...
    npArr = Turbulence.WebServices.getNumpyArrayFromRawResponse(self.dataset.name, self.field.accessorMethod, self.time, b_x, b_y, b_z, b_xwidth, b_ywidth, b_zwidth, self.field.dimensions, token=self.token)
    ...
    #print(npArr.shape)
    if(border > 0):
      resultStrided = npArr[border:-border:self.strideSpace,border:-border:self.strideSpace,border:-border:self.strideSpace,:]
    else:
  ...
  def checkBoundries(self, t, x, y, z, xwidth, ywidth, zwidth):
    ...
    if t < 0 or t >= (self.dataset.tsize):
      errorFlag = True
      errorMessage += "Time value t = " + str(t) + " is out of bounds.  It should be between 0 and " + str(self.dataset.tsize) + ".\n"
    if x < 0 or x >= self.dataset.xsize:
      errorFlag = True
      errorMessage = "Position x = " + str(x) + " is out of bounds.  It should be between 0 and " + str(self.dataset.xsize) + ".\n"
    if y < 0 or y >= self.dataset.ysize:
      errorFlag = True
      errorMessage = "Position y = " + str(y) + " is out of bounds.  It should be between 0 and " + str(self.dataset.ysize) + ".\n"
    if z < 0 or z >= self.dataset.zsize:
      errorFlag = True
      errorMessage = "Position z = " + str(z) + " is out of bounds.  It should be between 0 and " + str(self.dataset.zsize) + ".\n"
    if xwidth < 1 or (xwidth - x) > self.dataset.xsize:
      errorFlag = True
      errorMessage = "Width xwidth = " + str(xwidth) + " is out of bounds.  It should be between 0 and " + str((self.dataset.xsize - x)) + ".\n"
    if ywidth < 1 or (ywidth - y) > self.dataset.ysize:
      errorFlag = True
      errorMessage = "Width ywidth = " + str(ywidth) + " is out of bounds.  It should be between 0 and " + str((self.dataset.ysize - y)) + ".\n"
    if zwidth < 1 or (zwidth - z) > self.dataset.zsize:
      errorFlag = True
      errorMessage = "Width zwidth = " + str(zwidth) + " is out of bounds.  It should be between 0 and " + str((self.dataset.zsize - z)) + ".\n"
...
class Dataset:
  ...
  def getFieldReader(self, field, t=0, twidth=0, x=0, y=0, z=0, xwidth=0, ywidth=0, zwidth=0, token=""):
    ...
    reader = FieldReader(self, field, t, twidth, x, y, z, xwidth, ywidth, zwidth, self.strideTime, self.strideSpace, self.filter, token)
    return reader
  ...
  def getFilteredDataset(self, strideTime, strideSpace, filter):
    ...
    ds = Dataset(self.name, self.dt, self.tsize, self.xsize, self.ysize, self.zsize, self.fields, strideTime, strideSpace, filter, self.uid)
  ...
  def uploadToDatabase(self, newDatasetName, t=0, twidth=0, x=0, y=0, z=0, xwidth=0, ywidth=0, zwidth=0):
    ...
    turbTestToken = SciServer.Keystone.getToken(SciServer.Config.CasJobsTurbulenceUser, SciServer.Config.CasJobsTurbulencePassword, SciServer.Config.CasJobsTurbulenceTenant)
    ...
    for field in self.fields:
      ...
      sqlCreate = "CREATE TABLE " + SciServer.Config.CasJobsTurbulenceContext + "." + tableName +"(timestep int NOT NULL,zindex bigint NOT NULL, data varbinary(" + str(8*8*8*4*field.dimensions + 24) + ") NOT NULL)"
      
      #createResponse = SciServer.Databases.getReaderFromMyDB(sqlCreate)
      createResponse = SciServer.CasJobs.executeQuery(sqlCreate, context=SciServer.Config.CasJobsTurbulenceContext, token=turbTestToken)
      ...
      indexResponse = SciServer.CasJobs.executeQuery(sqlIndex,context=SciServer.Config.CasJobsTurbulenceContext, token=turbTestToken)
      if (createResponse == 500):
      ...
      while not fieldReader.closed:
        ...
        if resultStrided.shape != (coZSize,coYSize,coXSize,field.dimensions):
          print("Hey! Yo! The dimensions aren't right. ", resultStrided.shape, "!=", (coXSize,coYSize,coZSize,field.dimensions))
        else:
          for xindex in range(0, coXSize, ATOMLENGTH):
            for yindex in range(0, coYSize, ATOMLENGTH):
              for zindex in range(0, coZSize, ATOMLENGTH):
                i = i+1
                arrAtom = resultStrided[zindex:(zindex+ATOMLENGTH),yindex:(yindex+ATOMLENGTH),xindex:(xindex+ATOMLENGTH)]
                ...
                if (i >= uploadChunk):
                  ...
                  ts1 = time.time()
                  response = SciServer.CasJobs.uploadCVSDataToTable(str.encode(csvStr), tableName, SciServer.Config.CasJobsTurbulenceContext, token=turbTestToken)
                  ts2 = time.time()
      ...
      response = SciServer.CasJobs.executeQuery(createZIndexQuery,context=SciServer.Config.CasJobsTurbulenceContext,token=turbTestToken)

    print("Adding new dataset to turbinfo:",newDatasetName)
    addDatasetQuery = "EXEC dbo.spAddUserDataset '"+newDatasetName+"','"+schemaName+"',"+str(self.uid)+","+str(x)+","+str(y)+","+str(z)+","+str(xwidth)+","+str(ywidth)+","+str(zwidth)+","+str(t)+","+str(twidth)
  ...
  def writeToHFD5File(self, name, t=0, twidth=0, x=0, y=0, z=0, xwidth=0, ywidth=0, zwidth=0):
    ...
    h5file.create_array(h5file.root, "_size", obj=[int(twidth/self.strideTime),int(xwidth/self.strideSpace),int(ywidth/self.strideSpace),int(zwidth/self.strideSpace)])
...
def getDataset(name):
  ...
  for idx, pFrameField in pFrame.iterrows():
    ...
    if charname == 'u' or charname == 'p' or charname == 'b' or charname == 'a' or charname == 'd':
      ...
      accessorMethods = {'u' : 'rawvelocity', 'p' : 'rawpressure', 'b' : 'rawmagneticfield', 'a' : 'rawvectorpotential', 'd' : 'rawdesnity'}

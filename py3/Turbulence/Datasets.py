...
class FieldReader:
    ...
    def read(self):
        ...
        npArr = py3.Turbulence.WebServices.getNumpyArrayFromRawResponse(self.dataset.name, self.field.accessorMethod,
...
class Dataset:
    ...
    def uploadToDatabase(self, newDatasetName, t=0, twidth=0, x=0, y=0, z=0, xwidth=0, ywidth=0, zwidth=0):
        ...
        turbTestToken = py3.SciServer.LoginPortal.getToken(py3.Turbulence.Config.CasJobsTurbulenceUser,
                                                       py3.Turbulence.Config.CasJobsTurbulencePassword)
        schemaName = py3.SciServer.CasJobs.getSchemaName(turbTestToken)
        ...
        for field in self.fields:
            ...
            print("Creating table ", tableName, " to database context ", py3.Turbulence.Config.CasJobsTurbulenceContext)
            sqlCreate = "CREATE TABLE " + py3.Turbulence.Config.CasJobsTurbulenceContext + "." + tableName + "(timestep int NOT NULL,zindex bigint NOT NULL, data varbinary(" + str(
                8 * 8 * 8 * 4 * field.dimensions + 24) + ") NOT NULL)"

            #createResponse = SciServer.Databases.getReaderFromMyDB(sqlCreate)
            createResponse = py3.SciServer.CasJobs.executeQuery(sqlCreate,
                                                            context=py3.Turbulence.Config.CasJobsTurbulenceContext,
                                                            token=turbTestToken)

            if (createResponse == 500):
                print("There was an error creating the table: ", createResponse,
                      ". Does the table already exist?  If so, this will append to the existing data.")

            print("adding index", tableName, " to database context ", py3.Turbulence.Config.CasJobsTurbulenceContext)
            sqlIndex = "ALTER TABLE " + py3.Turbulence.Config.CasJobsTurbulenceContext + "." + tableName + " add constraint pk_" + tableName + " primary key clustered (timestep, zindex)"

            indexResponse = py3.SciServer.CasJobs.executeQuery(sqlIndex, context=py3.Turbulence.Config.CasJobsTurbulenceContext,
                                                           token=turbTestToken)
            if (createResponse == 500):
                        ...
                        for yindex in range(0, coYSize, ATOMLENGTH):
                            for zindex in range(0, coZSize, ATOMLENGTH):
                                ...
                                if (i >= uploadChunk):
                                    ...
                                    ts1 = time.time()
                                    response = py3.SciServer.CasJobs.uploadCVSDataToTable(str.encode(csvStr), tableName,
                                                                                      py3.Turbulence.Config.CasJobsTurbulenceContext,
                                                                                      token=turbTestToken)
                                    ts2 = time.time()
            ...
            response = py3.SciServer.CasJobs.executeQuery(createZIndexQuery,
                                                      context=py3.Turbulence.Config.CasJobsTurbulenceContext,
                                                      token=turbTestToken)
        ...
        response = py3.SciServer.CasJobs.executeQuery(addDatasetQuery, context='turbinfo', token=turbTestToken)
...
def getDataset(name):
    ...
    cvsResponse = py3.SciServer.CasJobs.executeQuery(queryString, context, token=turbTestToken)

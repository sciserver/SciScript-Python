...
class Callback(object):
    ...
    def fluxToggle(self, event):
        self.specObj.removeFlux()
        self.specObj.shownFig.draw()


def view(fileName, fileSource):
    coaddObj, zObj = driver.loadFITS(fileName, fileSource)
    # just show stuff
    specObj = SpecUtil(coaddObj, zObj)
    # specObj.shownFig.show()
    callback = Callback(specObj)
    axFluxToggle = plt.axes([0.81, 0.05, 0.1, 0.075])
    bFluxToggle = Button(axFluxToggle, 'flux')
    bFluxToggle.on_clicked(callback.fluxToggle)

    # def skylineToggle(self,event):

# import InlineViewer as viewer
